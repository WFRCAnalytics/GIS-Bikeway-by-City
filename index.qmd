...
## Process Bikeway Types to Categories
...
facility_levels <- c(
  ...
  "Shoulder bikeway (3A)",
  "Other bike route, unspecified (3)",
  "Trail or Pathway",
  "Not Available"
)
...
sf_ut_bikeways <- sf_ut_bikeways |>
  dplyr::mutate(
    facility1 = forcats::fct_relevel(facility1, facility_levels),
    facility2 = forcats::fct_relevel(facility2, facility_levels)
  )
```
...
bikeway_miles_by_city <- sf_ut_bikeways |>
    ...
    # Summarize total lane miles by city and facility type
    ...
    # Make facility types into columns
    tidyr::pivot_wider(
      names_from = facility,
...
bikeway_miles_by_city <- bikeway_miles_by_city |>
    dplyr::rowwise() |>
    dplyr::mutate(
      total_bike_lane_miles = sum(dplyr::c_across(!city_name))
    ) |>
    dplyr::ungroup()

# View the results
bikeway_miles_by_city |>
  dplyr::arrange(dplyr::desc(total_bike_lane_miles))
```
...
combined_miles_by_city <- road_miles_by_city |>
  ...
  dplyr::select(
    city_name,
    dplyr::all_of(facility_levels),   # bike lane categories
    total_bike_lane_miles,
    road_lane_miles
  )
