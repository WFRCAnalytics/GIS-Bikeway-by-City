...
```{r}
# Set method: TRUE to conduct spatial join, FALSE to use existing city column
use_spatial_join <- FALSE
...
bikeway_miles_by_city <- sf_ut_bikeways |>
    ...
    dplyr::select(city_name = city, facility1_class, facility2_class) |>
    ...
    (\(data) {
      if (!use_spatial_join) {
        # Keep existing city_name
        data
      } else {
        # Remove existing city_name and perform spatial join
        data |>
        dplyr::select(-city_name) |>
        sf::st_join(
          sf_ut_cities |>
